...
const handleResponses = async (req, res) => {
  ...
  try {
    ...
    const hasMissingCodexInstructions = isCodexCLI && !req.body?.instructions

    // 如果不是 Codex CLI 请求，或 Codex CLI 缺失 instructions，则进行适配
    if (!isCodexCLI || hasMissingCodexInstructions) {
      req.body = req.body || {}
      ...
      logger.info(
        hasMissingCodexInstructions
          ? '📝 Codex CLI request missing instructions, applying compatibility adaptation'
