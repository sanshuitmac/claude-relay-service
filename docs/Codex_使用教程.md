...
总的来说：修改两个文件和设置环境变量，重启vscode等即可。

**1）配置文件 `config.toml`**

注意：原来已存在的config.toml配置文件建议先备份。 

路径：`%USERPROFILE%\.codex\config.toml`          【C盘/用户/具体用户/.codex文件夹下】

在文件开头添加：下方配置文件model可按需填写：gpt-5.2 、gpt-5.2-codex 、gpt-5.3-codex；推理模式effort也可将high改xhigh
...
⚠️ 必须将 `OPENAI_API_KEY` 设置为 `null`，多余内容删除。否则 Codex 会优先使用它而忽略环境变量。
...
系统级环境变量（推荐）：（windows开始图标--右键---选择 Windows Power Shell（管理员） 打开  ）
...
配置方法同上方VSCode等插件一样，修改.codex目录下的 config.toml 和 auth.json 两个文件，设置环境变量即可。

具体配置内容见上方：点击跳转 [方式A - 步骤1：配置Codex](#win-a-step1)
...
下方配置文件model可按需填写：gpt-5.2 、gpt-5.2-codex 、gpt-5.3-codex；推理模式effort也可将high改xhigh
...
同上方 macOS 平台的 VSCode 等插件的配置，修改两个配置文件和设置环境变量，详细可见：点击跳转 [方式A - 步骤1：配置 Codex](#mac-a-step1)
